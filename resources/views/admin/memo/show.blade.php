...
@section('page-title', 'Admin - Memo Request Review - UTB HR Central')
...
<div class="content-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <h2 style="color: #2c3e50; margin: 0;">Admin - Memo Request Review</h2>
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('admin.memo.index') }}" style="background: #6c757d; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px; font-weight: 600;">
                ← Back to Requests
            </a>
        </div>
    </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 15px;">
                    @foreach($memo->form_files as $index => $file)
                        <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; border: 1px solid #dee2e6; text-align: center;">
                            ...
                            <a href="{{ route('admin.memo.download-form', ['memo' => $memo, 'fileIndex' => $index]) }}" 
        ...
        @if($memo->status === 'submitted')
            <div style="margin-bottom: 30px;">
                ...
                <div style="background: #d4edda; padding: 25px; border-radius: 8px; border: 1px solid #c3e6cb; margin-bottom: 20px;">
                    ...
                    <form action="{{ route('admin.memo.approve', $memo) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div style="margin-bottom: 15px;">
                            <label for="admin_notes" style="display: block; color: #155724; font-weight: 600; margin-bottom: 5px;">Admin Notes (Optional)</label>
                ...
                <div style="background: #f8d7da; padding: 25px; border-radius: 8px; border: 1px solid #f5c6cb;">
                    ...
                    <form action="{{ route('admin.memo.reject', $memo) }}" method="POST">
                        @csrf
                        <div style="margin-bottom: 15px;">
        ...
        @if($memo->status === 'approved' && $memo->memo_file)
            <div style="margin-bottom: 30px;">
                ...
                <div style="background: #d4edda; padding: 20px; border-radius: 8px; border: 1px solid #c3e6cb; text-align: center;">
                    ...
                    <a href="{{ route('admin.memo.download-memo', $memo) }}" 
